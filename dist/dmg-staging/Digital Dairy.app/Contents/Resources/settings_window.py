...
class SettingsEditor(ttk.Frame):
    """设置表单。`standalone=True` 时父级一般为 `tk.Tk`，会设置窗口标题与尺寸。"""

    def __init__(self, parent: tk.Misc, project_root: Path, *, standalone: bool = False) -> None:
        super().__init__(parent)
        self.project_root = project_root
        self._standalone = standalone

        if standalone and isinstance(parent, tk.Tk):
            parent.title("Digital Dairy 设置")
            parent.geometry("760x620")
            parent.minsize(680, 560)
    ...
    def _build_ui(self) -> None:
        ...
        if self._standalone:
            ttk.Button(bottom, text="关闭", command=self._close_window).pack(side="right", padx=(8, 0))
...
def main() -> int:
    ...
    root = tk.Tk()
    SettingsEditor(root, project_root, standalone=True).pack(fill="both", expand=True)
    root.mainloop()
    return 0
